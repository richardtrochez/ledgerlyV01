...
/**
 * @desc    Obtener Estado de Resultados comparativo por meses
...
export const getIncomeStatement = async (req, res) => {
  try {
    ...
    const companyId = req.query.companyId || '000000000000000000000001'

    if (startMonth < 1 || startMonth > 12 || endMonth < 1 || endMonth > 12) {
      return res.status(400).json({ success: false, message: 'Los meses deben estar entre 1 y 12' })
    }
    if (startMonth > endMonth) {
      return res.status(400).json({ success: false, message: 'El mes inicial no puede ser mayor al mes final' })
    }
    ...
    res.status(500).json({ success: false, message: 'Error al generar el Estado de Resultados', error: error.message })
  }
}

/**
 * @desc    Exportar Estado de Resultados en PDF
...
export const getIncomeStatementPDF = async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="estado-resultados-${fiscalYear}.pdf"`)
    ...
    doc.fillColor('white').fontSize(10).text(`Periodo: ${periodoLabel}`, 40, 58)
    doc.text(`Generado: ${new Date().toLocaleDateString('es-HN')}`, 400, 58, { align: 'right' })

    let y = 90

    // ─── TÍTULO ───────────────────────────────────────────────
    doc.fillColor(BLACK).fontSize(14).font('Helvetica-Bold').text('ESTADO DE RESULTADOS', 40, y, { align: 'center' })
    y += 25
    ...
    months.forEach((m, i) => {
      doc.text(m.name.substring(0, 3).toUpperCase(), startX + labelCol + i * colWidth, y + 6, { width: colWidth, align: 'right' })
    })
    if (months.length > 1) {
      doc.text('TOTAL', startX + labelCol + months.length * colWidth, y + 6, { width: 70, align: 'right' })
    }
    y += 22
    ...
    const drawSection = (section, color) => {
      ...
      section.costClasses.forEach((cc, rowIdx) => {
        ...
        cc.months.forEach((amt, i) => {
          doc.text(fmt(amt), startX + labelCol + i * colWidth, y + 4, { width: colWidth, align: 'right' })
        })
        if (months.length > 1) {
          doc.text(fmt(cc.total), startX + labelCol + months.length * colWidth, y + 4, { width: 70, align: 'right' })
        }
        y += 16
      })
      ...
      section.totals.forEach((amt, i) => {
        doc.text(fmt(amt), startX + labelCol + i * colWidth, y + 5, { width: colWidth, align: 'right' })
      })
      if (months.length > 1) {
        doc.text(fmt(section.grandTotal), startX + labelCol + months.length * colWidth, y + 5, { width: 70, align: 'right' })
      }
      y += 22
    }
    ...
    const drawSummaryRow = (label, values, grandTotal, color, bold = false) => {
      ...
      values.forEach((val, i) => {
        ...
        doc.text(fmt(val), startX + labelCol + i * colWidth, y + 5, { width: colWidth, align: 'right' })
      })
      if (months.length > 1) {
        const total = grandTotal !== undefined ? grandTotal : values.reduce((s, v) => s + v, 0)
        doc.fillColor(bold ? 'white' : (total >= 0 ? BLACK : RED))
        doc.text(fmt(total), startX + labelCol + months.length * colWidth, y + 5, { width: 70, align: 'right' })
      }
      y += 22
    }
    ...
    sections.forEach(section => {
      const colors = { ingreso: GREEN, costo_directo: '#d97706', gasto_operativo: '#dc2626', otros: GRAY }
    ...
    if (s.margenBruto?.length > 0) {
      drawSummaryRow('MARGEN BRUTO', s.margenBruto, s.margenBruto.reduce((a,b)=>a+b,0), '#d1fae5', false)
    }
    if (s.utilidadOperativa?.length > 0) {
      drawSummaryRow('UTILIDAD OPERATIVA', s.utilidadOperativa, s.utilidadOperativa.reduce((a,b)=>a+b,0), BLUE, true)
    }

    // ─── PIE DE PÁGINA ────────────────────────────────────────
    doc.rect(0, doc.page.height - 30, doc.page.width, 30).fill(BLUE)
    doc.fillColor('white').fontSize(8).font('Helvetica')
    doc.text('© 2026 Ledgerly Base — Sistema de Gestión Financiera para MiPymes', 40, doc.page.height - 18, { align: 'center' })
