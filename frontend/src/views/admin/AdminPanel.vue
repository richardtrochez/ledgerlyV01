<template>
  <div class="min-h-screen bg-gray-900">
    <!-- Header -->
    <div class="bg-gray-800 border-b border-gray-700">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div class="flex justify-between items-center">
          <h1 class="text-3xl font-bold text-white">Panel de Administración</h1>
          <button
            @click="logout"
            class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md text-sm font-medium"
          >
            Cerrar Sesión
          </button>
        </div>
      </div>
    </div>

    <!-- Contenido -->
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        
        <!-- Card Usuarios -->
        <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
          <h2 class="text-xl font-bold text-white mb-4">👥 Gestión de Usuarios</h2>
          <p class="text-gray-400 mb-4">Crear, editar y eliminar usuarios del sistema</p>
          <router-link
            to="/admin/usuarios"
            class="block w-full text-center bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md font-medium"
          >
            Ir a Usuarios
          </router-link>
        </div>

        <!-- Card Empresas -->
        <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
          <h2 class="text-xl font-bold text-white mb-4">🏢 Información Empresa</h2>
          <p class="text-gray-400 mb-4">Ver información de la empresa</p>
          <div class="bg-gray-700 rounded p-3 mb-4">
            <p class="text-sm text-gray-300">
              <span class="font-semibold">ID Empresa:</span> 000000000000000000000001
            </p>
            <p class="text-sm text-gray-300 mt-1">
              <span class="font-semibold">Usuario:</span> {{ userName }}
            </p>
          </div>
        </div>

      </div>
    </div>
  </div>
</template>

<script setup>
import { ref, onMounted } from 'vue'
import { useRouter } from 'vue-router'
import { useAuthStore } from '@/stores/auth'

const router = useRouter()
const authStore = useAuthStore()
const userName = ref('Admin')

onMounted(() => {
  console.log('AdminPanel cargado')
  console.log('User role:', authStore.user?.role)
  
  // Verificar que es admin
  if (authStore.user?.role !== 'admin') {
    console.log('No es admin, redirigiendo...')
    router.push('/dashboard')
    return
  }

  userName.value = authStore.user?.name || 'Admin'
})

function logout() {
  authStore.logout()
  router.push('/login')
}
</script>